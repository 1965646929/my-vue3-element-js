<template>
    <el-scrollbar>
        <el-menu
            ...
            :background-color="
                theme == 'dark' || sidebarColorTheme == SidebarColor.CLASS_BLUE
            ...
            <el-menu-item v-for="route in topMenus" :key="route.path" :index="route.path">
                <template #title>
                    <template v-if="route.meta && route.meta.icon">
                        <el-icon v-if="route.meta.icon.startsWith('el-icon')">
                            <component :is="route.meta.icon.replace('el-icon-', '')"></component>
                        </el-icon>
                        <div v-else :class="`i-svg:${route.meta.icon}`"></div>
                    </template>
...
//获取侧边栏颜色
const sidebarColorTheme = computed(() => settingsStore.sidebarColorTheme);
//Q:
// 获取当前路由路径的顶部菜单路径
...
onMounted(() => {
    console.log("mounted", permissionStore.routes);
